        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">
              {{ isEditing ? "Edit Share" : "Create Share" }}
            </h5>
  ...
  setup() {
    ...
    const loadShares = async () => {
      ...
      try {
        ...
        console.log("Shares response:", response); // Debug log
        shares.value = response.data || [];
      } catch (error) {
        console.error("Error loading shares:", error); // Debug log
    ...
    const editShare = (share) => {
      ...
      currentShare.value = {
        ...
        shareDate: share.shareDate
          ? new Date(share.shareDate).toISOString().split("T")[0]
          : "",
      };
      const modalElement = document.getElementById("shareModal");
      modalElement.classList.add("show");
