          ...
          <table class="table table-striped">
            ...
            <tbody>
              <tr v-for="viewHistory in viewHistories" :key="viewHistory.id">
                ...
                <td>
                  <button class="btn btn-sm btn-outline-primary me-2" @click="editViewHistory(viewHistory)">
                    <i class="fas fa-edit"></i>
                  </button>
                  <button class="btn btn-sm btn-outline-danger" @click="deleteViewHistory(viewHistory.id)">
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ isEditing ? 'Edit View History' : 'Create View History' }}</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          <div class="modal-body">
            <form @submit.prevent="saveViewHistory">
              <div class="mb-3">
                <label for="viewHistoryId" class="form-label">View History ID</label>
                <input 
                  type="number" 
                  class="form-control" 
                  id="viewHistoryId" 
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-warning" @click="saveViewHistory">
...
export default {
  ...
  setup() {
    ...
    const editViewHistory = (viewHistory) => {
      ...
      currentViewHistory.value = { 
        ...viewHistory,
        viewDate: viewHistory.viewDate ? new Date(viewHistory.viewDate).toISOString().split('T')[0] : ''
      }
      const modal = new bootstrap.Modal(document.getElementById('viewHistoryModal'))
      modal.show()
    }
    ...
    const saveViewHistory = async () => {
      try {
        if (isEditing.value) {
          await viewHistoryService.update(currentViewHistory.value.id, currentViewHistory.value)
          showNotification('success', 'Success', 'View history updated successfully')
        } else {
          await viewHistoryService.create(currentViewHistory.value)
          showNotification('success', 'Success', 'View history created successfully')
        }
        
        const modal = bootstrap.Modal.getInstance(document.getElementById('viewHistoryModal'))
        modal.hide()
        loadViewHistories()
      } catch (error) {
    ...
    const deleteViewHistory = async (id) => {
      if (confirm('Are you sure you want to delete this view history?')) {
        try {
          ...
          showNotification('success', 'Success', 'View history deleted successfully')
          loadViewHistories()
        } catch (error) {
